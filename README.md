...
- 1小節目の鍵盤表示を正しく修正
- 残りの小節もハイライト表示するように修正
- ハイライトカーソルを表示するようにした
- ハイライトカーソルの高さをscore.cssで正しくした

0066

- ロード時に前回読み込んだ楽譜があれば表示
- 楽譜読み込み中のローディングアニメーションを追加
- ズームのプリセットを配置
- ハイライトカーソルの移動で楽譜をスクロール

0067

- .musicxmlをローカルストレージに保存/削除する機能を追加
- ハイライトカーソルの色を修正/左手も右手も同じ
- 鍵盤の動的表示範囲を1オクターブ下を追加

0068

- コード自動解析を追加したけど精度が低くてキャンセル
- コードのオンオフボタンを追加
- 保存されたズーム値がリロード時に読み込まれない不具合の修正
- コードのオンオフ切り替え時にローディングを追加

0069

- ローディング画像をsvgに変更
- ローディング画面に名言を追加
- mfやmpなどの音楽用語をクリックするとモーダルで説明表示を追加
- ハイライトカーソルのジャンプ
- ダークモードに対応
- ハイライトカラーの修正
- 加線がダークモード時に黒いままな不具合
- 音符クリック時にダークモードだとジャンプしない不具合
- ダークモードの音符にホバーした時のカラーを修正
...
- 全画面表示ボタンを追加
- ハイライトカーソルが消える不具合の修正
- Time stretchやhands shakeは非表示

0072

- skillBlogs/pages/0015用のコンポーネントを追加
- skillBlogs/pages/016の追加と修正
- 練習モードの追加
...
- 落下ノートを表示する鍵盤モード追加
- 落下ノートが正しくない不具合の修正(gemini)2h
- claudeで修正0.5h
- 楽譜モードの色を一括管理するlib/colors.tsを作成

0077

- 鍵盤モードの落下ノートの長さ修正(merry-go-round-of-lifeの13小節目)
...
- skillBlogs/pages/0016を更新
- 見積書をpdfにして送信(見積書\_プランC-20260224.pdf)
- pdfのヘッダーは管理者のみに変更
...
- Excel VBA で画像やオブジェクトを多く扱うなら、**64bit 版の Excel をおすすめ**します。
