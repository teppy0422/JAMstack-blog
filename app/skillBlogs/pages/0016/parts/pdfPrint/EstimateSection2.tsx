...
import { ANNEX_SECTIONS, MARKET_RATES, ESTIMATE_NOTES } from "./annexContent";
...
export default function EstimateSection2({
  isAdmin,
  onPrintReady,
}: {
  isAdmin: boolean;
  ...
  const handlePrint = () => {
    ...
    const marketRateHtml = MARKET_RATES.map((r) =>
      `<tr><td style="color:#666; font-size:10px; padding:2px 8px;">${r.label}</td><td style="color:#666; font-size:10px; padding:2px 8px;">${r.value}</td></tr>`
    ).join("");
...
<style>
  @page { size: A4; margin: 15mm 20mm; @bottom-right { content: counter(page) " / " counter(pages); font-size: 10px; color: #333; } }
...
<p style="font-weight:bold; font-size:11px; margin:12px 0 4px;">単価根拠（市場相場）</p>
<table style="width:auto; margin:0;">
  <tbody>${marketRateHtml}</tbody>
</table>

<div class="footer">
  <p>【備考】</p>
  ${ESTIMATE_NOTES.map((n) => `<p>・${n}</p>`).join("")}
  <br/>
...
${annexPage}
  ...
  }, [devProjects, devTotal, planName, isAdmin]);

  return (
    <>
      <Text fontSize="sm" mb={2}>確認日:2026/2/10</Text>
      <Box mt={2}>
        ...
        >
          {MARKET_RATES.map((r, i) => (
            <React.Fragment key={i}>
              <GridItem>{r.label}</GridItem>
              <GridItem>{r.value}</GridItem>
            </React.Fragment>
          ))}
        </Grid>
        <Grid
        ...
        <Box mt={4} fontSize="xs" color="gray.600">
          <Text fontWeight="bold" mb={1}>【備考】</Text>
          {ESTIMATE_NOTES.map((n, i) => (
            <Text key={i}>・{n}</Text>
          ))}
          <Text mt={2}>※当方は適格請求書発行事業者ではありません。</Text>
          <Text>※当方は免税事業者です。</Text>
        </Box>
      </Box>
    </>
  );
